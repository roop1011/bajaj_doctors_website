...
const DoctorCard: React.FC<DoctorCardProps> = ({ doctor }) => {
  // Ensure all doctor properties are available or provide fallbacks
  const safeDoctor = {
    id: doctor.id || '',
    ...
    fee: doctor.fee || 0,
    reviews: {
      count: doctor.reviews?.count || 0,
      rating: doctor.reviews?.rating || 0
    },
    consultationMode: {
      videoConsult: !!doctor.consultationMode?.videoConsult,
      inClinic: !!doctor.consultationMode?.inClinic
    },
      ...
      <div className="flex flex-col md:flex-row">
        <div className="md:w-1/4 mb-4 md:mb-0 flex justify-center md:justify-start">
          <div className="h-24 w-24 md:h-32 md:w-32 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
            {safeDoctor.image ? (
              <img src={safeDoctor.image} alt={safeDoctor.name} className="h-full w-full object-cover" />
            ) : (
              <span className="text-4xl text-gray-400">{safeDoctor.name.charAt(0)}</span>
            )}
          </div>
        </div>
        <div className="md:w-3/4">
          <div className="flex flex-col md:flex-row md:justify-between">
            <div>
              ...
              <p data-testid="doctor-specialty" className="text-primary text-sm mb-2">
                {safeDoctor.specialty.join(', ')}
              </p>
              <p data-testid="doctor-experience" className="text-neutral-400 text-sm mb-2">
                <Clock className="inline-block mr-1 h-4 w-4" /> {safeDoctor.experience} years experience
              </p>
              <div className="flex items-center mb-2">
                <div className="bg-green-50 text-success text-xs font-medium px-2 py-1 rounded mr-2">
                  {safeDoctor.reviews.rating}% <ThumbsUp className="inline-block ml-1 h-3 w-3" />
                </div>
                <span className="text-neutral-400 text-sm">{safeDoctor.reviews.count} patient reviews</span>
              </div>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-neutral-100">
            <div className="flex flex-wrap gap-3">
              {safeDoctor.consultationMode.videoConsult && (
              ...
              {safeDoctor.consultationMode.inClinic && (
                <div className="bg-neutral-100 text-neutral-400 text-xs px-3 py-1 rounded-full">
                  <Building className="inline-block mr-1 h-3 w-3" /> In Clinic
                </div>
              )}
              {safeDoctor.languages.length > 0 && (
                <div className="bg-neutral-100 text-neutral-400 text-xs px-3 py-1 rounded-full">
                  <Stethoscope className="inline-block mr-1 h-3 w-3" /> {safeDoctor.languages.join(', ')}
                </div>
              )}
              {safeDoctor.location && (
                <div className="bg-neutral-100 text-neutral-400 text-xs px-3 py-1 rounded-full">
                  <MapPin className="inline-block mr-1 h-3 w-3" /> {safeDoctor.location}
                </div>
              )}
            </div>
          </div>
